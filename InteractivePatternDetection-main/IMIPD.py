...
def similarity_measuring_patterns(patterns_data, patient_data, pair_cases, start_search_points,
    ...
    try:
        ...
        iterator = tqdm(patterns_data['patterns'], desc="Calculating similarity")
    except ImportError:
        iterator = patterns_data['patterns']
    ...
    for pattern in iterator:
        ...
        # Optimized index calculation (O(1) instead of O(N) search)
        selected_pair_indices = []
        for a in in_pattern_cases:
            for b in out_pattern_cases:
                u, v = (a, b) if a < b else (b, a)
                # Formula matches construction: start_search_points[u] is offset for row u
                # v - u - 1 is offset within row u
                idx = int(start_search_points[u] + v - u - 1)
                selected_pair_indices.append(idx)

        if selected_pair_indices:
             patterns_data.loc[patterns_data['patterns'] == pattern, 'Case_Distance_Interest'] = \
                np.mean([pairwise_distances_array[ind] for ind in selected_pair_indices])
        else:
             patterns_data.loc[patterns_data['patterns'] == pattern, 'Case_Distance_Interest'] = 0.0
...
def Single_Pattern_Extender(all_extension_list, chosen_pattern_ID,
    ...
    for idx, case in enumerate(all_extension_list[chosen_pattern_ID]['Instances']['case']):
        ...
        if len(new_patterns_for_core) > 0 and not patient_data.empty:
